...
const COMPOSER = {
  ...
  continue_prompt: 'THIS IS FOR THE MODEL AND MUST NOT BE DRAWN',
};
...
test('the model\'s continuation rides on the card and is never drawn', () => {
  const node = composer();
  assert.ok(textOf(node).indexOf('THIS IS FOR THE MODEL') === -1);
  (function walk(el) {
    for (const [k, v] of Object.entries(el.attributes)) {
      assert.ok(String(v).indexOf('THIS IS FOR THE MODEL') === -1, `${el.tagName}[${k}] leaked the prompt`);
    }
    for (const c of el.children) walk(c);
  })(node);
});
